...
class DashboardController {
  async indexAppliedVaccines(req, res) {
    ...
    const listVaccineApplication = await Application.sequelize.query(
      `select vaccine_id, name from (
    ...
    for (let index = 0; index < listVaccineApplication.length; index++) {
      ...
      let dados = await Application.sequelize.query(
        `select EXTRACT(YEAR FROM ap.dt_aplicacao) as ano, 
  ...
  async indexCampaignVaccines(req, res) {
    ...
    const listCampaign = await Campaign.sequelize.query(
      `select c.descricao, 
                                                                c.vaccine_id,
                                                                c.dt_ini, c.dt_fim 
                                                           from campaigns c
    ...
    for (let index = 0; index < listCampaign.length; index++) {
      ...
      let dados = await Application.sequelize.query(
        `select EXTRACT(YEAR FROM ap.dt_aplicacao) as ano, 
                                                      ...
                                                      and EXTRACT(YEAR FROM ap.dt_aplicacao) = date_part('year', current_timestamp)
                                                      and ap.dt_aplicacao between TO_TIMESTAMP('${new Date(
                                                        element.dt_ini
                                                      ).toLocaleDateString(
                                                        "pt-BR"
                                                      )}', 'YYYY-MM-DD HH24:MI:SS') and TO_TIMESTAMP('${new Date(
          element.dt_fim
        ).toLocaleDateString("pt-BR")}', 'YYYY-MM-DD HH24:MI:SS')
                                                      group by ano, mes, ap.vaccine_id
                                                      having count(ap.*) > 0
                                                      order by ano, mes `,
        { type: QueryTypes.SELECT }
    ...
    let removeNull = JSON.stringify(dashboardCampaignVaccine)
      .replace(",null", "")
      .replace(",null,null", "");
