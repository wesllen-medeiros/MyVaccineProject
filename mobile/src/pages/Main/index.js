...
export default function Main() {
  ...
  async function loadNotification(userId) {
    let resultPermission = await Permissions.askAsync(
    ...
    if (notificacaoParaEnviar.length > 0) {
      for (let index = 0; index < notificacaoParaEnviar.length; index++) {
        ...
        if (Platform.OS === "android") {
          const channelCreated = await Notifications.createChannelAndroidAsync(
            "testAndroid",
            {
              name: "myVaccine",
  ...
  async function setContent(userData) {
    ...
    let day =
      parseInt(birthday.getDate()) < 10
        ? "0" + birthday.getDate()
        : birthday.getDate();
    let month =
      parseInt(birthday.getMonth() + 1) < 10
  ...
  async function getApplied(userId) {
    ...
    if (dataApplied.length > 0) {
      ...
      return {
        dt_aplicacao: "Não houveram aplicações até o momento",
          ...
          >
            ...
            >
              <Top>
                ...
                <CardItem>
                  <Avatar.Image
                    style={{ marginRight: 15, backgroundColor: "#34b7f1" }}
                    size={180}
                    source={
                      photoProfile
                        ? { uri: photoProfile }
